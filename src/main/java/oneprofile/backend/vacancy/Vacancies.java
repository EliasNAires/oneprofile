...
public class Vacancies {
	...
	public Vacancies(VacancyRepository repository, CompanyRepository companies) {
	 ...
	 */
	@Transactional
	public Reconciliation mirror(Ats ats, String slug, Collection<PublishedVacancy> published) {
		Company company = this.companies.findByAtsAndSlug(ats, slug)
		...
		Map<Long, Vacancy> held = new HashMap<>();
		for (Vacancy vacancy : this.repository.findByCompany(company)) {
			held.put(vacancy.externalId(), vacancy);
		}
		int added = 0;
		List<Vacancy> mirrored = new ArrayList<>(published.size());
		for (PublishedVacancy opening : published) {
			Vacancy vacancy = held.remove(opening.externalId());
			if (vacancy == null) {
				vacancy = new Vacancy(company, opening.externalId());
				added++;
			}
