 ...
 */
@Component
public class TitleCleaningRule {
	...
	private static final List<SeniorityWord> SENIORITY_WORDS = List.of(
			new SeniorityWord(word("semi[\\s-]?senior|ssr"), SeniorityLevelEnum.MID),
			new SeniorityWord(word("senior|sr"), SeniorityLevelEnum.SENIOR),
			new SeniorityWord(word("junior|jr"), SeniorityLevelEnum.JUNIOR),
			new SeniorityWord(word("principal"), SeniorityLevelEnum.PRINCIPAL));
	...
	public CleanedTitle withoutSeniorityWords(String title) {
		Set<SeniorityLevelEnum> named = new LinkedHashSet<>();
		String left = title;
	...
	private record SeniorityWord(Pattern pattern, SeniorityLevelEnum level) {
	}
