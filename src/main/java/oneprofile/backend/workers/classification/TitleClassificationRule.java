 ...
 */
@Component
public class TitleClassificationRule {
	 ...
	 * <p>
	 * An undecided ruling is always {@link UnknownReasonEnum#SCOPE_AMBIGUITY}: a ruled phrase left
	...
	private static final Map<String, Classification> MANAGEMENT_FAMILIES = Map.ofEntries(
			...
			Map.entry("technical project manager", Classification.unknown(UnknownReasonEnum.DOMAIN_AMBIGUITY)),
			Map.entry("technical project managers", Classification.unknown(UnknownReasonEnum.DOMAIN_AMBIGUITY)),
	...
	public Classification classify(String cleanedTitle) {
		...
		if (GENERIC_HEADS.contains(head) && words.contains("technical")) {
			...
			return modified ? scopeAmbiguity() : Classification.unknown(UnknownReasonEnum.DOMAIN_AMBIGUITY);
		}
		...
		return reading.engineeringCapable() ? Classification.unknown(UnknownReasonEnum.DOMAIN_AMBIGUITY) : unruled();
	}
	...
	private static Classification unruled() {
		return Classification.unknown(UnknownReasonEnum.UNRULED);
	}
